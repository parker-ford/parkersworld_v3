...
<script>
	...
	let code01 = `    const fontLoader = new FontLoader();
    fontLoader.load(
        fonts["path-to-font"],
        (font) => {
	...
	let code02 = `    textMesh = new THREE.Mesh(textGeometry, material);
    scene.add(textMesh);
    `;

	let code03 = `    textGeometry.computeBoundingBox()
...
<div class="page__main">
	<div class="page__content">
		...
		<div class="page__text__container">
			...
			<p class="page__body">
				In order to create 3d text in threejs, you need to use the font loader. The font loader can
				load fonts that are in a typeface format. From there you can create a new textGeometry and
				specify its font and several other parameters. Then you can use this new geometry to create
				a mesh like you normally would. One thing to note is that this will place the text geometry
				with its bottom left point at the world origin. In order to center it on the screen, you
				must do a calculation based on its bounding box.
			</p>
			...
			<p class="page__body">
				Matcaps are a special kind of material that can be used in 3D rendering to create a unique
				look for objects. Instead of using traditional lighting and shading techniques, matcaps use
				a pre-rendered texture that is wrapped around the object's surface to create the appearance
				of depth and detail. These textures are designed to look like different materials, such as
				plastic, metal, or skin, and can be applied to a variety of objects to give them a specific
				look and feel. They allow for a performant lighting simulation without the need of an actual
				lighting setup.
				<br />
				<br />
			...
			<p class="page__body">
				...
				<br />
				Matcap textures sourced from:
				<a href="https://github.com/nidorx/matcaps">https://github.com/nidorx/matcaps</a>
			</p>
		</div>
	</div>
</div>
