...
export class UVMaterial extends Material {
    ...
    constructor(options) {
        super();
        this.id = UVMaterial.count++;
        this.color = options.color ? options.color : vec4.fromValues(1, 0, 0, 1);
    }

    init(options){
        this.topology = options.wireframe ? 'line-list' : 'triangle-list';
        if(!UVMaterial.bindGroupLayout){
            UVMaterial.bindGroupLayout = this.createBindGroupLayout();
        }
        if(!UVMaterial.pipelines[this.topology]){
            UVMaterial.pipelines[this.topology] = this.createPipeline(options);
        }
